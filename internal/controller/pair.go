...
func (r *BurnInRunReconciler) startPair(
	...
	pod, buildErr := podForTest(run, index, attempt, t.Name, &t.Spec, t.Axes, serverNode, r.vendorFor(ctx, serverNode), run.Spec.Target, &rendezvous{
...
func (r *BurnInRunReconciler) gateClientOnServer(
	...
	if _, terminated, _, _ := podOutcome(serverPod); terminated {
		...
		r.harvestArtifacts(ctx, run, t.Name, nodes, serverPod.Name, parsed.Artifacts)
		return advanceHarvested, advanceEffect{dirty: true}, nil
	}
	...
	pod, buildErr := podForTest(run, index, attempt, t.Name, &t.Spec, t.Axes, clientNode, r.vendorFor(ctx, clientNode), run.Spec.Target, &rendezvous{
		scope:    burninv1alpha1.ScopePair,
		role:     pairRoleClient,
...
func (r *BurnInRunReconciler) harvestPair(
	...
	r.harvestArtifacts(ctx, run, t.Name, nodes, clientPod.Name, clientParsed.Artifacts)
