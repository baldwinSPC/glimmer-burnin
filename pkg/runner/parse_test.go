...
`,
		...
		},
		{
			...
			// the runner now prints devices_visible (registered devicesVisible,
			// see docs/dev/multi-device.md). The capture stays as captured:
			// what this asserts is the generic snake→camel path, and gpuCount
			// is deliberately NOT aliased, because two keys must never alias to
			// one name.
			kind: "host-health",
