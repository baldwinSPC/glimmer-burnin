...
func TestANamedInterfaceIsCheckedAgainstTheRouteAndNotTrusted(t *testing.T) {
	// THE HOLE THIS CLOSES. TCP_BASELINE_INTERFACE was used as the whole answer
	// whenever it was set: the guard compared the STRING against the management
	// interface and never asked where the kernel would actually send.
	//
	// Nothing binds iperf3 to that interface — no --bind is passed, and the
	// runner deliberately does not pass one, because the kernel's routing table
	// is the authority on where a packet goes. So an author naming a fabric
	// interface on a node whose route to the peer is the management path got a
	// clean bill of health from the guard and then put the load on the
	// management path anyway. That is the exact outcome the guard exists to
	// prevent, reached through the guard.
	//
	// config/samples/tcp-baseline.yaml sets this variable, and BurnInTest env
	// applies to BOTH pods of a pair, so the shipped sample is the configuration
	// that disabled the real check on the deciding end.
	res := reconcileIface("enp1s0f0", "eth0")
	if res.decision != routeUnknown {
		t.Fatalf("naming enp1s0f0 while the route leaves via eth0 was accepted: %+v", res)
	}
	for _, want := range []string{"enp1s0f0", "eth0"} {
		if !strings.Contains(res.reason, want) {
			t.Errorf("the refusal does not name %q, so nobody can act on it: %q", want, res.reason)
		}
	}
}

func TestANamedInterfaceThatAgreesWithTheRouteIsUsed(t *testing.T) {
	res := reconcileIface("enp1s0f0", "enp1s0f0")
	if res.decision != routeOK || res.iface != "enp1s0f0" {
		t.Fatalf("agreement was refused: %+v", res)
	}
}

func TestWithNoRouteToCrossCheckTheNamedInterfaceStands(t *testing.T) {
	// The SERVER end. Its peer does not exist yet — the operator does not create
	// the client until the server is Ready, so the client's DNS record is not
	// resolvable and ifaceForAddr can answer nothing. There is no contradiction
	// to detect, and the named interface is all the guard has.
	res := reconcileIface("enp1s0f0", "")
	if res.decision != routeOK || res.iface != "enp1s0f0" {
		t.Fatalf("the server's named interface was refused: %+v", res)
	}
}

func TestWithNothingNamedTheRouteDecides(t *testing.T) {
	res := reconcileIface("", "eth3")
	if res.decision != routeOK || res.iface != "eth3" {
		t.Fatalf("the routed interface was not used: %+v", res)
	}
}
