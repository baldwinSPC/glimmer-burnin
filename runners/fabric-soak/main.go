...
func envInt(name string, def int) int {
	...
	if err != nil || n <= 0 {
		logf("fabric-soak: %s=%q is not a positive integer; using %d", name, v, def)
		return def
	}
	return n
}
...
func run() int {
	...
	if peerHost == "" {
		return fin(exitError, "BURNIN_ROLE=%s but BURNIN_PEER_HOST is empty — there is no peer", role)
	}
